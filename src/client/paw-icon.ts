 ...
 * @module dsh-live2d-pets/client/paw-icon
 */
...
export function PawPrintIcon(props: { size?: number; className?: string }): ReactElement {
  ...
  return createElement(
    ...
    createElement('path', { fillRule: 'evenodd', d: PAW_PATH }),
  )
}

/** 设置.section 注册用：按尺寸渲染爪印（待平台投影 icon 字段后可直接生效）。 */
export function pawNavIcon(size: number): ReactNode {
  return createElement(PawPrintIcon, { size })
}
 ...
 */
export function installPetSettingsNavIcon(): () => void {
  const swap = (): void => {
    for (const btn of document.querySelectorAll('button')) {
      const label = Array.from(btn.querySelectorAll('span')).find((s) => s.textContent?.trim() === NAV_LABEL)
      if (!label) continue
